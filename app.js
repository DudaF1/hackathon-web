/* app.js — Bloco 1 (Otimizado e Revisado) */
...
function funcionarios(){ return state.usuarios.filter(u => (u.tipo || '').toLowerCase() === 'funcionario'); }
function admins(){ return state.usuarios.filter(u => (u.tipo || '').toLowerCase() === 'admin'); }
...
function bindShell(){
  ...
  $('#profileBtn').onclick = () => $('#profileDropdown').toggle('hidden');
...
async function carregarDadosDoBanco() {
  try {
    const response = await fetch('https://sua-api.onrender.com/api/usuarios');
    if (response.ok) {
      const dadosReais = await response.json();
      state.usuarios = dadosReais.map(u => ({
        nome: u.nome,
        usuario: u.identificador,
        tipo: (u.tipo || 'funcionario').toLowerCase(),
        setor: u.setor || u.nomeSetor || 'Geral',
        cargo: u.funcao || 'Operador',
        senha: '1234'
      }));
      render();
    }
...
function renderBenefitDetail(id){
  ...
  app.innerHTML = `<button class="outline-btn" id="backBenefits">Voltar</button>${heading(map[id], 'Gestão administrative e solicitações recebidas')}${benefitInfo(id)}<section class="card" style="margin-top:16px"><h3 class="card-title">Solicitações recebidas</h3><div class="list">${pedidos.map(s => requestItem(s)).join('') || '<p>Nenhuma solicitação encontrada.</p>'}</div></section>`;
  $('#backBenefits').onclick = () => { state.selectedBenefit = null; save(); renderBeneficios(); };
  if(id==='valeTransporte') bindTransport();
}
/* app.js — Bloco 2 (Otimizado e Revisado) */
...
window.showDependentes = () => openModal('Dependentes Santa Helena','Inclusões e exclusões recebidas', `<div class="list">${(state.dependentesConvenio||[]).map(d => listItem(d.nome, `${d.funcionario} • ${d.parentesco}<br>CPF: ${d.cpf} • Nasc.: ${d.nascimento}<br>Documentos: ${(d.documentos || []).join(', ')}`, d.status)).join('') || '<p>Nenhum dependente cadastrado.</p>'}</div>`);
...
function renderPessoal(){
  ...
  const items = [['ferias','Controle de férias','Férias, retornos e PDF.'],['holerites','Holerites','Cópias assinadas e pendências.'],['faltas','Faltas e atestados','Justificativas e apontamentos.'],['exames','Exames periódicos','ASO, pendências e datas.'],['treinamentos','Treinamentos','Conteúdos e各自 conclusões.'],['solicitacoes','Solicitações gerais','Todos os pedidos do portal.']];
...
function pessoalInfo(id){
  ...
  if(id==='holerites') return `<div class="list">${(state.holeritesAdmin || []).map(f => { const p=f.holerites.filter(h=>h.status==='Esperando assinatura').length; return listItem(f.nome, `${f.setor} • ${f.cargo}<br>${f.holerites.length} holerites • ${p} pendente(s)`, p?`${p} pendente(s)`:'OK', `showHolerites('${jsArg(f.nome)}')`); }).join('')}</div>`;
...
function holeriteItem(funcionario, holerite){
  ...
  const detalhes = `Valor líquido: ${text(holerite.valorLiquido || 'R$ 0,00')}<br>Protocolo: ${text(holerite.protocoloDocusign || 'Pendente')}${holerite.dataAssinatura ? `<br>Assinado em: ${text(holerite.dataAssinatura)}` : ''}<div class="item-actions">${holeriteActions(funcionario, holerite)}</div>`;
...
function lineChart(data){
  ...
  return `<svg class="line-chart" viewBox="0 0 ${w} ${h}" role="img"><defs><linearGradient id="g" x1="0" y1="0" y2="1"><stop stop-color="#6b4226" stop-opacity=".18"/><stop offset="1" stop-color="#6b4226" stop-opacity="0"/></linearGradient></defs><path d="${d} L ${lastPt[0]},${h-p} L ${firstPt[0]},${h-p} Z" fill="url(#g)"/><path d="${d}" fill="none" stroke="#6b4226" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>${pts.map((pt,i)=>`<circle cx="${pt[0]}" cy="${pt[1]}" r="6" fill="#3f2618"/><text x="${pt[0]}" y="${pt[1]-12}" text-anchor="middle" fill="#6b4226" font-weight="800" font-size="14">${data[i]}%</text>`).join('')}</svg>`;
}
...
function renderPerfil(){
  ...
  const u = currentUser || admins()[0] || {nome: 'Admin', usuario: 'admin', setor: 'RH'};
  const avatarInitials = String(u.nome || 'AD').split(' ').map(p=>p[0]).slice(0,2).join('');
  app.innerHTML = `${heading('Perfil do admin','Configurações e dados profissionais')}<div class="profile-cover"></div><section class="card profile-card"><div class="avatar">${avatarInitials}</div><h2>${u.nome}</h2><p class="card-sub">${u.setor} • Administrador do portal</p><div class="grid three"><div class="card"><strong>Usuário</strong><p>${u.usuario}</p></div><div class="card"><strong>Idade</strong><p>${u.idade || '-'} anos</p></div><div class="card"><strong>Transportadora</strong><p>${u.transportadora?.nome||'-'}</p></div></div></section>`;
}
